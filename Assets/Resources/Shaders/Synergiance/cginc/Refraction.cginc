...
float3 refractGrab(float3 normalDir, float4 screenPos, float3 viewDir) {
	...
	float red = tex2D(_RefractGrab, (tempCoords + cameraRefraction)).r;
	float green = tex2D(_RefractGrab, (tempCoords + (cameraRefraction * (1.0 - _ChromaticAberration)))).g;
	float blue = tex2D(_RefractGrab, (tempCoords + (cameraRefraction * (1.0 + _ChromaticAberration)))).b;
