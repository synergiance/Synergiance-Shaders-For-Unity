...
BoxProjectData calcBoxProjectData(float3 viewDir, float3 normal, float3 posWorld) {
	BoxProjectData bpd;
...
float3 BoxProject(BoxProjectData i) {
	...
	float3 calcBlankCubeDetectionMultiplier(float3 lightCol, float atten) {
		float3 lightContrib = lightCol + ShadeSH9(half4(0,0,0,1));
		float probeVal = calcProbeVal(lightCol, atten);
		return lightContrib * probeVal * atten;
	}
#endif

fixed3 sampleCubeRefl(BoxProjectData bpd, float roughness, float3 bcd) {
	fixed3 probe = 0;
...
fixed3 calcProbe(float3 viewDir, float3 normal, float3 posWorld, float3 lightCol, float roughness, float atten) {
	BoxProjectData bpd = calcBoxProjectData(viewDir, normal, posWorld);

	#ifdef BLANK_CUBE_DETECTION
		float3 bcd = calcBlankCubeDetectionMultiplier(lightCol, atten);
	#else
		float3 bcd = 0;
	#endif

	return sampleCubeRefl(bpd, roughness, bcd);
}
...
fixed3 refractProbe(float3 viewDir, float3 normal, float3 posWorld, float3 lightCol, float clarity, float indexOfRefraction, float atten) {
	...
	return sampleCubeRefl(bpd, 1 - clarity, bcd);
}
